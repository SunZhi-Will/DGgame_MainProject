...
public class PlayerAI : MonoBehaviour
{
    ...
    void Moving()
    {
        if (PlayerControl_koroshi.Key(PlayerNumber.p1, PlayerKeyName.Right) && rb.position.x > boundaryManager.xMin)
        {
            animator.SetBool("Running", true);
            v_moveXpos = Vector3.left.x * PlayerSpeed * Time.deltaTime;
        }
        else if (PlayerControl_koroshi.Key(PlayerNumber.p1, PlayerKeyName.Left) && rb.position.x < boundaryManager.xMax)
        {
            animator.SetBool("Running", true);
        ...
        if (PlayerControl_koroshi.Key(PlayerNumber.p1, PlayerKeyName.Down))
        {
            animator.SetBool("Running", true);
            v_moveZpos = Vector3.forward.z * PlayerSpeed * Time.deltaTime;
        }
        else if (PlayerControl_koroshi.Key(PlayerNumber.p1, PlayerKeyName.Up))
        {
            animator.SetBool("Running", true);
    ...
    //void Moving()
    //{
    //    //if (Input.GetKey(KeyCode.Semicolon) && rb.position.x > boundaryManager.xMin)
    //    if (PlayerControl_koroshi.Key(PlayerNumber.p1, PlayerKeyName.Right) && rb.position.x > boundaryManager.xMin)
    //    {
    //        animator.SetBool("Running", true);
    //        transform.Translate(Vector3.left * PlayerSpeed * Time.deltaTime + Vector3.forward * PlayerSpeed * Time.deltaTime);
    //    }
    //    //else if (Input.GetKey(KeyCode.K) && rb.position.x < boundaryManager.xMax)
    //    else if (PlayerControl_koroshi.Key(PlayerNumber.p1, PlayerKeyName.Left) && rb.position.x < boundaryManager.xMax)
    //    {
    //        animator.SetBool("Running", true);
    ...
    void Jumping()
    {
        ////if (IsFloor && Input.GetKeyDown(KeyCode.Period))
        //if (IsFloor && PlayerControl_koroshi.Key(PlayerNumber.p1, PlayerKeyName.Cancel))
        //    {
    ...
    void Pushing()
    {
        //if (Input.GetKey(KeyCode.I)&& InitialSpeed == 1)
        if (PlayerControl_koroshi.Key(PlayerNumber.p1, PlayerKeyName.Confirm) && InitialSpeed == 1)
        {
            animator.SetBool("Pull", true);
        }
        //else if (Input.GetKey(KeyCode.I))
        if (PlayerControl_koroshi.Key(PlayerNumber.p1, PlayerKeyName.Confirm))
        {
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        else if (other.tag == "Win")
        {
            ...
            gameData_Parkour_scr.ClosePlayerControl();//關閉玩家控制器1P
        }
    }
